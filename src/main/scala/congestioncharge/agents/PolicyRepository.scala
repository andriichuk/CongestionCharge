...
class PolicyRepository extends Actor {
  ...
  private val _carPolicy = ReceiptPolicy(VehicleType.Car, List(
    ReceiptPolicyRule("AM rate", 2, WeekDays.WorkWeek, LocalTimeInterval(new LocalTime(7, 0), new LocalTime(12, 0))),
    ReceiptPolicyRule("PM rate", 2.5, WeekDays.WorkWeek, LocalTimeInterval(new LocalTime(12, 0), new LocalTime(19, 0)))
    )
  )
  private val _motorbikePolicy = ReceiptPolicy(VehicleType.Motorbike, List(
    ReceiptPolicyRule("AM rate", 1, WeekDays.WorkWeek, LocalTimeInterval(new LocalTime(7, 0), new LocalTime(12, 0))),
    ReceiptPolicyRule("PM rate", 1, WeekDays.WorkWeek, LocalTimeInterval(new LocalTime(12, 0), new LocalTime(19, 0)))
    )
  )
